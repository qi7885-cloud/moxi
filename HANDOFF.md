...
| 空山创作区 | `mount` | 第壹章画布；点击即地起峰（峰顶=落指处，含画布顶边）：`sil()` 脊线沿主峰外随机生 2~4 座子峰起伏相接，细碎锯齿破直线；四种山形（尖峰/叠嶂/石壁/缓丘）随机且不与前峰重样（`lastSp`）；落指越高山越远墨越淡（`near`），近山才皴（石壁小斧劈、余者披麻）点苔；峰多则 `deco` 收敛笔触，上限 60 座旧者移除；渲染按远→近排序叠压；山脚雾带统一收底 |
...
| 声音 | `sound` / `pluck()` | 双振荡器（tri + 倍频 sine）→ 低通扫频 → 反馈延迟回声；`SCALE` 为 A 起五声音阶（宫商角徵羽）；**默认开启**，`unlockAudio()` 在首次 pointerdown/keydown（捕获阶段）解锁 AudioContext 并拉起音量（浏览器手势策略）；「抚水有声」按钮可手动开关 |
| 惯性卷动 | `applyScroll()` | 不劫持滚轮：内容容器 `#smooth` 为 fixed，rAF 里对原生 `scrollY` 做 lerp 追随，`#spacer` 撑起真实页高。滚动条/键盘/锚点天然可用，无障碍友好 |
| 导航与光标 | `rail` / `[data-magnet]` / `#cursor` | 右侧卷轴进度线（墨填充 + 四章锚点）；磁性按钮；自定义光标三态：`link`（悬停可点）/ `ink`（落墨/起峰）/ `touch`（抚水）——三态文字标签均用同款细线放大圈（`scale(2.2)`）；`pointer: coarse` 自动隐藏 |
...
- [ ] 山章：空山画布点击起峰——峰顶即落指处（含画布顶边）、脊线蜿蜒、连点不重样、多点后仍不杂乱；RESET 清空
- [ ] 声章：抚水出涟漪与音符（默认有声，首次点击后）、墨鲤循声而来、受惊而散；开关可关可开
